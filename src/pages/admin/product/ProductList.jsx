...
export default function ProductList() {
  ...
  useEffect(() => {
    ...
    async function loadProducts() {
      ...
      try {
        const data = await getServices({ token, page, limit: PAGE_SIZE, search })
  ...
  const handleRestore = async (product) => {
    ...
    try {
      await restoreService({ token, id: product._id })
  ...
  return (
    <AdminLayout>
      <PageHeader
        ...
        subtitle="Add each product/service once, then pick it from a dropdown in quotations and invoices to autofill the description, unit and rate."
        action={addButton}
      />
      ...
      <div className="bg-white border border-gray-100 rounded-[22px] shadow-sm mt-8 overflow-hidden">
        ...
        {loading ? (
          ...
          <EmptyState
            ...
            description="Add a product or service once here, and it will autofill in quotations and invoices."
            action={addButton}
          />
        ) : (
          <>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-6 py-4 border-t border-gray-100">
              <p className="text-sm text-[#64748B]">
                Showing <span className="font-semibold text-[#0F172A]">{products.length}</span> of{" "}
